...
export function useItems() {
    ...
    async function fetchItems() {
        try {
            const { data, error } = await supabase.from('items').select('id, name, image_url, category, referencia_petitorio');
...
export function useTechniques() {
    ...
    async function fetchTechniques() {
        try {
            ...
            const { data, error } = await supabase
                .from('techniques')
                .select(`
        id, name, category, description, protocol_url, icon_name, cart_ids,
        technique_items (
          quantity,
          items (id, name, image_url, category, referencia_petitorio)
        ),
        technique_equipment (
          quantity,
          equipment (id, name, description, image_url, category, stock_quantity, maintenance_status, location, requires_power, created_at)
        )
      `);
    ...
    async function createTechnique(technique: { name: string; description: string; protocolUrl?: string; cartIds?: string[]; items: { itemId: string; quantity: number }[]; equipment: { equipmentId: string; quantity: number }[] }) {
        ...
        try {
            ...
            if (techError) {
                console.error("Create technique DB Error:", techError);
                throw techError;
            }

            // 2. Create Technique Items
            if (technique.items && technique.items.length > 0) {
                const techniqueItems = technique.items.map(item => ({
                    technique_id: techData.id,
                    item_id: item.itemId || (item as any).item_id || (item as any).item?.id,
            ...
            if (technique.equipment && technique.equipment.length > 0) {
                const techniqueEquipment = technique.equipment.map(eq => ({
                    technique_id: techData.id,
                    equipment_id: eq.equipmentId || (eq as any).equipment_id || (eq as any).equipment?.id,
...
export function useEquipment(unitId?: string) {
    ...
    async function fetchEquipment() {
        try {
            let query = supabase.from('equipment').select('id, name, description, image_url, category, stock_quantity, maintenance_status, location, requires_power, created_at, unit_id').order('name');
            if (unitId) {
